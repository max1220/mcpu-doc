...
## JavaScript parser & JSON documentation

The `json/` directory contains a dump of every possible instruction and ALU operation in JSON format. It's generated by the `gen_instructions.js` and `gen_alu_instructions.js` node scripts, which make use of the definitions in `mcpu_defs.js` and `mcpu_alu_defs.js`. These scripts also serve as a sort of reference instruction decoder.
